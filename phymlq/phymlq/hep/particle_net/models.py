"""
Holds the code for the actual model that calls on all the custom layers
"""
...
class ParticleNet(tf.keras.models.Model):
    ...
    def __init__(self,
        ...
        """
        assert model_type in ['particlenet-lite',
                              'particlenet-full'], 'Model Name not in list.'
        ...
        self.settings = {
            ...
            'conv_pooling':
            'average',
            'fc_params': [(128, 0.1)] if model_type == 'particlenet-lite' else
            [(256, 0.1)] if model_type == 'particlenet-full' else [],
            'num_points':
            input_shapes['points'][0]
        }

        self.layer_points = tf.keras.Input(name='points',
                                           shape=input_shapes['points'])
        ...
        self.layer_mask = tf.keras.Input(
            name='mask',
            shape=input_shapes['mask']) if 'mask' in input_shapes else None
    ...
    def fit_dataset(self,
                    ...
                    batch_size=None,
                    epochs=10,
                    checkpoint_file='checkpoints/best.h5',
                    tensorboard_dir='logs/'):
        """
        Runs the model and trains it on a dataset.
        Note that the raw model.fit has not been overridden
        """
        batch_size = 1024 if 'lite' in self.name else 384
        if val_dataset is not None:
            val_dataset = (val_dataset.net, val_dataset.y)

        def lr_schedule(epoch):
            return 1e-3 if epoch <= 10 else 1e-4 if epoch <= 20 else 1e-5

        dataset.shuffle()
        self.fit(dataset.net,
                 dataset.y,
    ...
    def _make_model(self):
        ...
        with tf.name_scope('particle_net'):
            ...
            fts = tf.keras.layers.BatchNormalization(name='bn_features')(
                self.layer_features)

            for layer_idx, layer_param in enumerate(
                    self.settings['conv_params']):
                k_count, channels = layer_param
                pts = MaskOut(True, name='mask_%d' % (layer_idx))([
                    self.layer_points if layer_idx == 0 else fts,
                    self.layer_mask
                ])
                fts = EdgeConvolution(self.settings['num_points'],
                                      k_count,
                                      channels,
                                      with_bn=True,
                                      activation='relu',
